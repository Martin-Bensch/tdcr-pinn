...
public:
	...
	Eigen::Matrix<bool, Eigen::Dynamic, 1> forwardKinematics_pyb(int poses,
                                     pybind11::EigenDRef<const Eigen::VectorXd>
                                      q,
                                     pybind11::EigenDRef<const Eigen::VectorXd>
                                      f_ext,
                                     pybind11::EigenDRef<const Eigen::VectorXd>
	...
	Eigen::MatrixXd getEEFrames(); // returns all frames for the given poses vector
	...
	Eigen::MatrixXd getDiskFrames_storage();
	Eigen::MatrixXd getDiskStates();
	Eigen::MatrixXd getDiskStates_storage_segment1();
	Eigen::MatrixXd getDiskStates_storage_segment2();
	...
	Eigen::MatrixXd getTendonDisplacements_storage();

	// ################
	// Added function for wrapper
	CosseratRodModel getCosseratRodModel();
	SubsegmentCosseratRodModel getSubsegmentCosseratRodModel();
	Eigen::MatrixXd getDefaultInitValues_uv(Model enum_model);
	Eigen::MatrixXd getFinalInitValues_uv(Model enum_model);
	void setDefaultInitValues_uv(Eigen::MatrixXd inits_uv);
	void reset_last_inits();
	Eigen::MatrixXd get_uv_s0_storage();
	Eigen::MatrixXd getStateSegmentN_CR(int segment);

	void set_gxf_tol_step_size(float tol, double step_size);
...
private:
	...
	// Added
	Eigen::MatrixXd m_tendon_displacements_storage;
	...
	// Added
	Eigen::MatrixXd m_ee_frame_storage;
	Eigen::MatrixXd m_uv_s0_storage;

	Eigen::MatrixXd m_disk_frames;
	//Added
	Eigen::MatrixXd m_disk_frames_storage;
	Eigen::MatrixXd m_disk_states;
	Eigen::MatrixXd m_disk_states_storage_segment1;
	Eigen::MatrixXd m_disk_states_storage_segment2;
