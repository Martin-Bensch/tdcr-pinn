...
#include "../include/cosseratrodmodel.h"
...
int differential_equations_cr(double t, const double input[2], double deriv[2], void *param)
{
    ...
    // Where R_float is R_dot stored as a vector.
    Eigen::MatrixXd dy;
...
void CosseratRodModel::run_IVP(Eigen::MatrixXd &states, Eigen::MatrixXd init_state)
{
    ...
    //double h = 0.005;
    //Fixed step length
    double h = m_step_size; //t1/m_number_disks[m_current_segment-1];
    //double h_init = m_step_size;//t1/m_number_disks[m_current_segment-1];
    double h_init = t1/m_number_disks[m_current_segment-1];

    //std::cout << "h " << h << " h_init " << h_init << std::endl;
    for(int i = 0; i < 19; i++)
    {
        y[i] = init_state(0,i);
    }
    ...
    // Calculates the state vector for each disk
    ...
    for(int i = 0; i < m_number_disks[m_current_segment-1]; i++)
    {
        ...
        std::cout << "t1 " << t1 << " t " << t << std::endl;
        std::cout << "for loop - m_step_size h " << h << std::endl;
        */
        int status;
        while (abs(t1 - t) > h * 0.01)
        {
            //Variable step length
           // status = gsl_odeiv2_evolve_apply (m_ode_evolve, m_ode_controller, m_ode_step, &m_ode_sys, &t, t1, &h, y);
            ...
            {
                //std::cout << "Break: " << status << std::endl;
                break;
                }
        }
       // std::cout << "t, t1: " << t << " " << t1 << "   t - t1 " << t - t1 << " status: " << status << std::endl;
        if (!(status != GSL_SUCCESS))
        {
            ...
            if (abs(t1 - t) <= m_step_size * 0.01)
            {
                y_stacked.push_back(temp);
            }/*else{
                std::cout << "Not stacked t1 - t " << t1 - t << std::endl;
             }*/
        }
    }
...
bool CosseratRodModel::forwardKinematics(Eigen::MatrixXd &diskFrames, Eigen::MatrixXd q, Eigen::Vector3d f_ext, Eigen::Vector3d l_ext)
{
    ...
    if(m_keep_inits)
    {
        ...
        for (int i = 0; i < m_last_inits.cols(); i++) {
            if (std::abs(inits(0, i)) < 1e-8) {
                inits(0, i) = 0;
            }
        }
    }
   ...
   // std::cout << "Keep inits: " << m_keep_inits << "\n";
   ...
   // std::cout << "\n";
    ...
    const double xtol = m_gxftol;//1.0e-10;
    const double gtol = m_gxftol;//1.0e-10;
    const double ftol = m_gxftol;//1.0e-10;
...
void CosseratRodModel::get_res(Eigen::MatrixXd &output, Eigen::MatrixXd input)
{
    ...
    // Sum of equation (18) in RucRob2011 for all 3 tendons per segment
    ...
    // Kse is the stiffness matrix for shear & extending and Kbt for bending &
    // torsion
