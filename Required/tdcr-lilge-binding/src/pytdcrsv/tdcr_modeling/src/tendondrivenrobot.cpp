...
#include "../include/tendondrivenrobot.h"
#include <iostream>
...
void TendonDrivenRobot::setRobotParameters(std::array<double, 2> length, double youngs_modulus, std::vector<Eigen::Vector3d> routing, std::array<int, 2> number_disks, std::array<double, 2> pradius_disks, double ro, bool two_tendons)
{
    ...
    // std::cout << "Updated parameters in tendondrivenrobot.cpp" << std::endl;
...
TendonDrivenRobot::~TendonDrivenRobot()
{
    // std::cout << " Destructor in TendonDrivenRobots"  << std::endl;
    delete mp_cr_model;
    delete mp_cc_model;
    delete mp_pcc_model;
    delete mp_prb_model;
...
bool TendonDrivenRobot::forwardKinematics(Eigen::Matrix4d &ee_frame,
                                          Eigen::MatrixXd q,
                                          Eigen::Vector3d f_ext,
                                          Eigen::Vector3d l_ext, Model model)
{
    ...
    switch(model) {
    case CosseratRod:
        // m_state1 and m_state2 hold the last full states, for all disks.
...
Eigen::MatrixXd TendonDrivenRobot::getEEFrames()
{
    return m_ee_frame_storage;
}
...
Eigen::MatrixXd TendonDrivenRobot::getDiskStates()
{
    return m_disk_states;
}

Eigen::MatrixXd TendonDrivenRobot::getDiskFrames_storage()
{
    return m_disk_frames_storage;
}

Eigen::MatrixXd TendonDrivenRobot::getDiskStates_storage_segment1()
{
    std::cout << "getDisksStates storage segmnet1";
    std::cout << m_disk_states_storage_segment1;
    return m_disk_states_storage_segment1;
}

Eigen::MatrixXd TendonDrivenRobot::getDiskStates_storage_segment2()
{
    return m_disk_states_storage_segment2;
}

Eigen::MatrixXd TendonDrivenRobot::getTendonDisplacements_storage()
{
    return m_tendon_displacements_storage;
}
